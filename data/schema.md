...
| `type`           | string          | yes                  | `"normal"` or `"base"`. `"base"` shapes contain a bay/cove or a mountain barrier to shield a base ship and are used once per side (independently, at random) when generating a map - see `baseAnchor` below and CLAUDE.md's "Map generation". A `"base"` shape may also turn up elsewhere on the map like any `"normal"` shape (at that spot's usual random scale/rotation). |
...
| `baseAnchor`     | `{x, y}`        | only for `type: "base"` | The point in this shape's local space where the base ship sits - outside `landShape`/`mountainShapes` (open water), typically in a bay, cove, or gap between landmasses. When a `"base"` shape is chosen as one side's homebase island, the map generator solves for the placement (position) that lands this exact point on that player's fixed `BASE_SHIP_START` (`js/engine/rules.js`), at a fixed scale (`HOMEBASE_ISLAND_SCALE`) and rotation (`HOMEBASE_ROTATION`) rather than a random one - see `solveHomebaseIslandPlacement()` in `js/engine/rules.js`. Author every `"base"` shape's land mass sitting up-and-right of `baseAnchor`, as if for player 1's bottom-left corner; the fixed per-player rotation mirrors it correctly for player 2's top-right corner automatically. |
